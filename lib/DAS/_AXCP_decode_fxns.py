...
from ._AXCP_convert_fxns import calc_temp_from_freq
from lib.DAS.common_DAS_functions import dataconvert
...
def init_AXCP_settings(self, settings):
    ...
    self.settings = {}
    self.settings["cprefreshrate"] = 1 #size of raw audio chunks to process, in seconds
    self.settings["revcoil"]     = False #coil on AXCP reversed- rotates currents by 180 degrees
    self.settings["axcpquality"]     = 1    #profile processing quality- 1=high/slow, 2=moderate speed/quality, 3=low/fast
    self.settings["spindowndetectrt"] = True #realtime detection of probe spindown to avoid processing unnecessary data
...
def calc_current_datapoint(self, t1, t2):
    ...
    tavg = np.round(np.nanmean(tss), 3)
    # tbof = tss(end)
    ...
    depth =  np.round(dataconvert(timd,self.depth_poly), 1)
    ...
    temp = np.round(self.calc_temp_from_freq(ftbl, depth), 3)
...
def iterate_AXCP_process(self, e): 
    ...
    self.T = np.append(self.T, np.round(e/self.f_s,2)) #time at end of current PCM chunk
    ...
    if self.status: 
        ...
        while process_new_depth:
            ...
            if t2 > self.tim[-1]:
                ...
                utrue, vtrue = self.calculate_true_velocities(umag, vmag)
                ...
                cur_Vmag.append(vmag)
                cur_Utrue.append(utrue)
                cur_Vtrue.append(vtrue)
                ...
                self.U_TRUE = np.append(self.U_TRUE, utrue)
                self.V_TRUE = np.append(self.V_TRUE, vtrue)
                ...
                self.txtfile.write(f"nff={self.nff},{tavg},{depth},{temp_zc},{umag},{vmag},{area},{rotfavg},{rotfrms},{ftbl},{efbl},{ccbl},{fefr},{fccr},{terr},{verr},{aerr},{w},{envxccss},{pkss},{vc0a},{vc0p},{ve0a},{ve0p},{gcca},{gefa},{nindep}\n")
...
def refine_spindown_prof(self):
    ...
    if len(good_areas) >= 10:
        ...
        self.txtfile.write(f"Area refined: {self.amean_calc}\n")
...
def calculate_true_velocities(self, umag, vmag):
    
    #convert current profile from degrees magnetic to true
    vel = np.sqrt(umag**2 + vmag**2)
    curdir_true = (np.pi/180) * (90 - (180/np.pi) * np.arctan2(vmag, umag) + self.dec) #MEAT: magnetic -> east add -> true
    utrue = np.round(vel * np.sin(curdir_true),3)
    vtrue = np.round(vel * np.cos(curdir_true),3)
    return utrue, vtrue
